...
FQL_V1_RE = re.compile(r"\[FQL,\s*Online,\s*V1\s*\(Sparse,\s*Dense\),\s*SEED\s+\d+\]", re.IGNORECASE)
FQL_V30_RE = re.compile(
    r"\[FQL,\s*Online,\s*V30\s*\(Sparse,\s*Dense\),\s*SEED\s+\d+\]", re.IGNORECASE
)


def label_from_run(run_name: str) -> str | None:
    if FQL_V30_RE.search(run_name):
        return "Reward Shaped (Human)"
    if FQL_V1_RE.search(run_name):
        return "Sparse"
    return None
...
def plot_modern(agg: list[dict], out_png: Path) -> None:
    colors = {
        "Reward Shaped (Human)": "#dc2626",  # red
        "Sparse": "#2563eb",  # blue
    }
    ...
    ax.set_title("FQL [Online]", fontsize=17, fontweight="bold", pad=12)
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-csv",
        default="qam/analysis/cube_task_double_algo_fql_v1_vs_v30_eval_success_mean_std.csv",
    )
    parser.add_argument(
        "--out-png",
        default="qam/analysis/cube_task_double_algo_fql_v1_vs_v30_modern.png",
    )
    args = parser.parse_args()

    rows = collect_rows(args.entity, args.project, args.metric, args.samples_per_run)
    if not rows:
        raise RuntimeError("No matching FQL V1/V30 runs found in W&B.")
